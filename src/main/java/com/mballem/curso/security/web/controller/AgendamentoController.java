...
public class AgendamentoController {
	...
	@GetMapping("/agendar")
	public String agendarConsultas(Agendamento agendamento) {
	...
	public ResponseEntity<?> getHorarios(@PathVariable("id") Long id,
		...
		return ResponseEntity.ok(this.service.buscarHorariosNaoAgendados(id, data));
	}

	@PreAuthorize("hasAuthority('PACIENTE')")
	@PostMapping("/salvar")
	public String salvar(Agendamento agendamento, RedirectAttributes attr, @AuthenticationPrincipal User user) {
		Paciente paciente = this.pacienteService.buscarporEmail(user.getUsername());
	...
	public String historico(Agendamento agendamento) {
	...
	public ResponseEntity<?> historicoPorPaciente(HttpServletRequest request, @AuthenticationPrincipal User user) {
		if (user.getAuthorities().contains(new SimpleGrantedAuthority(PerfilTipo.PACIENTE.getDesc()))) {

			return ResponseEntity.ok(this.service.buscarHistoricoPorPacienteEmail(user.getUsername(), request));

		}

		if (user.getAuthorities().contains(new SimpleGrantedAuthority(PerfilTipo.MEDICO.getDesc()))) {
			return ResponseEntity.ok(this.service.buscarHistoricoPorMedicoEmail(user.getUsername(),request));
		}
	...
	public String preEditarConsultaPaciente(@PathVariable("id")Long id, ModelMap model, @AuthenticationPrincipal User user) {
		...
		Agendamento a  = this.service.findByIdAndUsuario(id,user.getUsername());
