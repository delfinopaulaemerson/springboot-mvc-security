...
public class MedicoController {
	...
	public String salvar(Medico medico, RedirectAttributes attr, @AuthenticationPrincipal User user) {
		if (medico.hasNotId() && medico.getUsuario().hasNotId()) {
			Usuario usuario = this.usuarioService.findByEmail(user.getUsername());
	...
	public String excluir(@PathVariable("idMed")Long idMed,@PathVariable("idEsp")Long idEsp, RedirectAttributes attr) {
		
		if(this.service.existeEspecialidadesAgendada(idMed, idEsp)) {
			...
			attr.addFlashAttribute("sucesso", "Especialidade realizada com sucesso!");
		}
	...
	public ResponseEntity<?> getMedicoPorEspecialidades(@PathVariable("titulo")String titulo) {

		return ResponseEntity.ok(this.service.buscarMedicoPorEspecialidades(titulo));
	}
